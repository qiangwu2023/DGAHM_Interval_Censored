...
beta_node_D = np.array([52, 45]) 
beta_lr_D = np.array([2e-4, 2e-4])
gamma_node_D = np.array([40, 27])
...
ax1_1_Survival.set_title("Case 4, n=500, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax1_2_Survival.set_title("Case 4, n=1000, $\Lambda_0(t)=t/8$", fontsize=10) 
...
# ------(prediction)---(Z=1,X=X1) fig 5_Survival is used to save the conditional survival function under two samples (each graph has two methods)------

fig2_Survival = plt.figure()
ax2_1_Survival = fig2_Survival.add_subplot(1, 2, 1)
ax2_1_Survival.set_title("Case 4, n=500, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax2_2_Survival.set_title("Case 4, n=1000, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax3_1_Survival.set_title("Case 4, n=500, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax3_2_Survival.set_title("Case 4, n=1000, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax4_1_Survival.set_title("Case 4, n=500, $\Lambda_0(t)=t/8$", fontsize=10) 
...
ax4_2_Survival.set_title("Case 4, n=1000, $\Lambda_0(t)=t/8$", fontsize=10) 
...
for i in range(len(Set_n)):
    ...
    sd_beta_D = np.sqrt(Sigma_D[0,0]) # standard error for \hat\beta 
    sd_gamma_D = np.sqrt(Sigma_D[1,1]) # standard error for \hat\gamma
    # save Ese
    ...
    sd_beta_L = np.sqrt(Sigma_L[0,0]) # standard error for \hat\beta 
    sd_gamma_L = np.sqrt(Sigma_L[1,1]) # standard error for \hat\gamma
    # save Ese
...
#%% -----------Save all results------------
# ================Figures======================
fig1_Survival.savefig('fig_Survival_Z0_X1_case4.jpeg', dpi=400, bbox_inches='tight')
fig2_Survival.savefig('fig_Survival_Z1_X1_case4.jpeg', dpi=400, bbox_inches='tight')
fig3_Survival.savefig('fig_Survival_Z0_X2_case4.jpeg', dpi=400, bbox_inches='tight')
fig4_Survival.savefig('fig_Survival_Z1_X2_case4.jpeg', dpi=400, bbox_inches='tight')
# =================Tables=======================
# results for beta
...
result_error_beta.to_csv('beta_error_case4.csv')
# results for beta
...
result_error_gamma.to_csv('gamma_error_case4.csv')
# results for g1
...
result_re_g1.to_csv('Re_g1_case4.csv')
# results for g2
...
result_re_g2.to_csv('Re_g2_case4.csv')
