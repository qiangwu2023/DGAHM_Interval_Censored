...
beta_node_D = np.array([40, 42]) 
beta_lr_D = np.array([2e-4, 2e-4]) 
gamma_node_D = np.array([52, 52]) 
gamma_lr_D = np.array([2e-5, 3e-5]) 
beta_node_A = np.array([50, 50]) 
beta_lr_A = np.array([3e-4, 5e-4]) 
gamma_node_A = np.array([45, 45]) 
gamma_lr_A = np.array([1e-5, 1e-5])
beta_node_L = np.array([50, 50]) 
beta_lr_L = np.array([3e-4, 5e-4]) 
...
ax1_1_Survival.set_title("Case 3, n=500, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax1_2_Survival.set_title("Case 3, n=1000, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax2_1_Survival.set_title("Case 3, n=500, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax2_2_Survival.set_title("Case 3, n=1000, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax3_1_Survival.set_title("Case 3, n=500, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax3_2_Survival.set_title("Case 3, n=1000, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax4_1_Survival.set_title("Case 3, n=500, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
ax4_2_Survival.set_title("Case 3, n=1000, $\Lambda_0(t)=t^{3/4}/6$", fontsize=10)
...
for i in range(len(Set_n)):
    ...
    n_lr = Set_lr[i]
    m = Set_m[i]
    nodevec = np.array(np.linspace(0, 91, m+2), dtype="float32")
    ...
    for b in range(B):
        ...
        Iu_D = I_U(m, np.clip(U_train * Ezg1_D, 0, 91), nodevec)
        Iv_D = I_U(m, np.clip(V_train * Ezg1_D, 0, 91), nodevec)
        Lamb_U_D = np.matmul(Iu_D, Est_D['C'])
        Lamb_V_D = np.matmul(Iv_D, Est_D['C'])
        f_U_D = Lamb_U_D * Ezg2_D
        f_V_D = Lamb_V_D * Ezg2_D
        
        Bu_D = B_S2(m, np.clip(U_train * Ezg1_D, 0, 91), nodevec)
        Bv_D = B_S2(m, np.clip(V_train * Ezg1_D, 0, 91), nodevec)
        ...
        # compute \Lambda()
        Iu_A = I_U(m, np.clip(U_train * Ezg1_A, 0, 91), nodevec)
        Iv_A = I_U(m, np.clip(V_train * Ezg1_A, 0, 91), nodevec)
        Lamb_U_A = np.matmul(Iu_A, Est_A['C'])
        Lamb_V_A = np.matmul(Iv_A, Est_A['C'])
        f_U_A = Lamb_U_A * Ezg2_A
        f_V_A = Lamb_V_A * Ezg2_A
        # compute \Lambda'()
        Bu_A = B_S2(m, np.clip(U_train * Ezg1_A, 0, 91), nodevec)
        Bv_A = B_S2(m, np.clip(V_train * Ezg1_A, 0, 91), nodevec)
        ...
        Iu_L = I_U(m, np.clip(U_train * Ezg1_L, 0, 91), nodevec)
        Iv_L = I_U(m, np.clip(V_train * Ezg1_L, 0, 91), nodevec)
        Lamb_U_L = np.matmul(Iu_L, Est_L['C'])
        Lamb_V_L = np.matmul(Iv_L, Est_L['C'])
        f_U_L = Lamb_U_L * Ezg2_L
        f_V_L = Lamb_V_L * Ezg2_L
        
        Bu_L = B_S2(m, np.clip(U_train * Ezg1_L, 0, 91), nodevec)
        Bv_L = B_S2(m, np.clip(V_train * Ezg1_L, 0, 91), nodevec)
...
#%% -----------Save all results------------
# ================Figures======================
fig1_Survival.savefig('fig_Survival_Z0_X1_case3(non-linear).jpeg', dpi=400, bbox_inches='tight')
fig2_Survival.savefig('fig_Survival_Z1_X1_case3(non-linear).jpeg', dpi=400, bbox_inches='tight')
fig3_Survival.savefig('fig_Survival_Z0_X2_case3(non-linear).jpeg', dpi=400, bbox_inches='tight')
fig4_Survival.savefig('fig_Survival_Z1_X2_case3(non-linear).jpeg', dpi=400, bbox_inches='tight')
# =================Tables=======================
# results for beta
...
result_error_beta.to_csv('beta_error_case3(non-linear).csv')
# results for beta
...
result_error_gamma.to_csv('gamma_error_case3(non-linear).csv')
# results for g1
...
result_re_g1.to_csv('Re_g1_case3(non-linear).csv')
# results for g2
...
result_re_g2.to_csv('Re_g2_case3(non-linear).csv')
