...
def g_L(train_data,X_test,X_subject,C,m,nodevec):
    ...
    def GL(*args):
        ...
        Iu = I_U(m, np.clip(U_train * np.exp(Z_train * b[2*d+2] + g1_X), 0, 83), nodevec)
        Iv = I_U(m, np.clip(V_train * np.exp(Z_train * b[2*d+2] + g1_X), 0, 83), nodevec)
        ...
        return loss_fun
    g_linear_para = spo.minimize(GL,np.zeros(2*d+4),method='SLSQP')['x']
