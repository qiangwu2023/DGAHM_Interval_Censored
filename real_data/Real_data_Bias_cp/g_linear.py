...
def g_L(Z_train,X_train,U_train,V_train,De1_train,De2_train,De3_train,X_sort1,X_sort0,theta,C,m,nodevec):
    c = Z_train.shape[1]
    d = X_train.shape[1]
    def GL(*args):
        b = args[0]
        ...
        Iu = I_U(m, U_train * np.exp(np.dot(Z_train, theta[0:c]) + g1_X), nodevec)
        Iv = I_U(m, V_train * np.exp(np.dot(Z_train, theta[0:c]) + g1_X), nodevec)
    ...
    g1_test1 = np.dot(X_sort1, g_linear_para[0:d]) + g_linear_para[d]*np.ones(X_sort1.shape[0])
    g2_test1 = np.dot(X_sort1, g_linear_para[(d+1):(2*d+1)]) + g_linear_para[2*d+1]*np.ones(X_sort1.shape[0])
    g_test1 = np.c_[g1_test1, g2_test1]

    g1_test0 = np.dot(X_sort0, g_linear_para[0:d]) + g_linear_para[d]*np.ones(X_sort0.shape[0])
    g2_test0 = np.dot(X_sort0, g_linear_para[(d+1):(2*d+1)]) + g_linear_para[2*d+1]*np.ones(X_sort0.shape[0])
